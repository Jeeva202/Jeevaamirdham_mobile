...
const BookDetailScreen: React.FC = () => {
  ...
  // Fetch user-specific data (favorites and cart)
  useEffect(() => {
    if (userId) {
      const fetchUserData = async () => {
        try {
          const favoritesResponse = await axios.get(`${REACT_API_URL}/favorites`, {
            params: { userId }
          });
          ...
          const cartResponse = await axios.get(`${REACT_API_URL}/ebooks/get_cart?id=${userId}`);
          const cartList = cartResponse.data.cart_details.map((item: { book_id: any; }) => item.book_id) || [];
          setIsInCart(cartList.includes(bookId));
        } catch (error) {
  ...
  // Update related books' isFavorite when favoritesList changes
  useEffect(() => {
    if (relatedBooks.length > 0) {
      setRelatedBooks(prev => prev.map(b => ({
        ...b,
  ...
  const toggleFavorite = useCallback(async () => {
    ...
    try {
      if (isFavorite) {
        await axios.post(`${REACT_API_URL}/deleteFavorites`, { userId, bookId });
        setFavoritesList(prev => prev.filter(id => id !== bookId));
        setIsFavorite(false);
        setSnackbarMessage('Removed from favorites');
      } else {
        await axios.post(`${REACT_API_URL}/addFavorites`, { userId, bookId });
        setFavoritesList(prev => [...prev, bookId]);
        setIsFavorite(true);
  ...
  const toggleFavoriteForRelated = useCallback(async (relatedBookId: number) => {
    if (!userId) {
      setSnackbarMessage('Please log in to manage favorites');
      setSnackbarVisible(true);
      return;
    ...
    try {
      const isCurrentlyFavorite = favoritesList.includes(relatedBookId);
      if (isCurrentlyFavorite) {
        await axios.post(`${REACT_API_URL}/deleteFavorites`, {
          userId,
          bookId: relatedBookId
        });
        setFavoritesList(prev => prev.filter(id => id !== relatedBookId));
        setSnackbarMessage('Removed from favorites');
      } else {
        ...
        setFavoritesList(prev => [...prev, relatedBookId]);
        setSnackbarMessage('Added to favorites');
      }
      setSnackbarVisible(true);
    } catch (error) {
      console.error("Error toggling favorite for related book:", error);
      setSnackbarMessage('Failed to update favorites');
      setSnackbarVisible(true);
    }
    ...
    >
      ...
      <View style={styles.relatedBookInfo}>
        ...
        <View style={styles.relatedBookFooter}>
          ...
          <TouchableOpacity onPress={() => toggleFavoriteForRelated(item.id)}>
            <IconButton
